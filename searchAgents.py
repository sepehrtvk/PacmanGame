...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "state returns an array with 2 parts. first is current node and second is visited corners until now."
        currentNode = state[0]
        exploredCorners = state[1]

        "we have all 4 corners of the maze in self.corners"
        if currentNode in self.corners:
            ...
            "check if visited all corners"
            explordLenght = len(exploredCorners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            "check if the new position hits a wall or not"
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]

            "add successors if we dont hit walls"
            if not hitsWall:
